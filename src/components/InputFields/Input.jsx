...
export const Input = ({ label, ...props }) => {
    ...
    return (
        <div className="w-full">
            <label className="block mb-2 text-sm font-medium text-gray-800" htmlFor={props.id || props.name}>
                {label}
            </label>
            <input
...
export const Radio = ({ label, ...props }) => {
    ...
    return (
        <div className="flex items-center justify-start gap-1">
            <input {...field} {...props} />
            <label className="text-sm text-gray-800" htmlFor={props.id || props.name}>
                {label}
            </label>
            {meta.touched && meta.error ? <p className="text-xs text-red-600 mt-1 italic">{meta.error}</p> : null}
        </div>
